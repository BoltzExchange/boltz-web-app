...
import { quoteDexAmountOut } from "./client";
...
import { assetAmountToSats, satsToAssetAmount } from "./evmAmount";
import { type EncodedHop, HopsPosition } from "./swapCreator";
...
const isRoutedAsset = (asset: string) =>
    getConfig().assets?.[asset]?.token?.routeVia !== undefined ||
    isUsdt0Asset(asset);
...
export class Pair {
    ...
    constructor(
        ...
        if (getConfig().assets?.[from]?.canSend === false) {
            return;
        }
        ...
        this.preOft =
            isUsdt0Variant(from) &&
            routeSource === USDT0 &&
            getConfig().assets?.[routeSource]?.network?.chainId !== undefined
                ? {
                      from,
                      to: routeSource,
                      destinationChainId:
                          getConfig().assets![routeSource]!.network!.chainId,
                  }
                : undefined;
        const postOft =
            isUsdt0Variant(to) &&
            routeTarget === USDT0 &&
            getConfig().assets?.[to]?.network?.chainId !== undefined
                ? {
                      from: routeTarget,
                      to,
                      destinationChainId:
                          getConfig().assets![to]!.network!.chainId,
                  }
                : undefined;
        ...
        const toAsset = getConfig().assets?.[routeTarget];
        if (toAsset !== undefined && toAsset.type === AssetKind.ERC20) {
            const hopAssetSymbol = toAsset.token?.routeVia;
            if (hopAssetSymbol === undefined) {
                // no routeVia — skip ERC20→DEX branch
            } else {
            const hopAsset = getConfig().assets?.[hopAssetSymbol];
            const hopPair = Pair.findPair(pairs, routeSource, hopAssetSymbol);

            if (
                hopPair !== undefined &&
                canUseRoutedBoltzHop(hopPair) &&
                hopAsset !== undefined
            ) {
                const sym = hopAssetSymbol;
                this.route = [
                    {
                        type: hopPair.type,
                        pair: hopPair.pair,
                    ...
                    },
                    {
                        ...
                        to: routeTarget,
                        dexDetails: {
                            chain: hopAsset.network!.symbol as string,
        ...
        const fromAsset = getConfig().assets?.[routeSource];
        if (fromAsset !== undefined && fromAsset.type === AssetKind.ERC20) {
            ...
            const hopAsset = getConfig().assets?.[hopAssetSymbol];
            const hopPair = Pair.findPair(pairs, hopAssetSymbol, routeTarget);
                ...
                this.route = [
                    {
                        ...
                        to: sym,
                        dexDetails: {
                            chain: fromAsset.network!.symbol as string,
    ...
    private get preOftMessagingFeeToken() {
        return this.preOft !== undefined
            ? getConfig().assets?.[this.preOft.from]?.network?.gasToken
            : undefined;
    }

    private get postOftMessagingFeeToken() {
        return this.postOft !== undefined
            ? getConfig().assets?.[this.postOft.from]?.network?.gasToken
            : undefined;
    }
    ...
    private get postOftFeeQuoteDetails():
        ...
        const claimAssetConfig = getConfig().assets?.[claimAsset];
        const chain =
    ...
    public getMinimum = async (): Promise<number> => {
        ...
        if (boltzHop.type !== SwapType.Submarine) {
            ...
            const p = boltzHop.pair as SubmarinePairTypeTaproot;
            boltzSendLimit = calculateSendAmount(
                BigNumber(
                    p.limits.minimalBatched || p.limits.minimal,
                ),
                this.feePercentage,
                this.minerFees,
                boltzHop.type,
