...
//     bun run examples/routeArkadeToUsdcBase.ts
//
// ── Environment ──────────────────────────────────────────────────────────────
...
//
// ⚠️  This spends real mainnet Arkade. The preimage and refund key printed below
//     are the ONLY way to refund the Arkade if the swap does not complete — copy
//     them somewhere safe before funding.
//
...
const main = async () => {
    ...
    // Secrets needed to claim (preimage) and to refund the Arkade if it fails.
    const preimage =
    ...
    console.log("Fund the Arkade lockup from your Arkade wallet:");
    ...
    if (lockup.blindingKey !== undefined) {
        // Needed (together with the refund key + preimage) to refund the lockup.
...
void main().catch((err) => {
    ...
    console.error(
        "If you already funded the Arkade lockup, use the printed preimage and " +
            "refund key to recover it after the timeout.",
    );
    process.exit(1);
});
