...
describe("deposit engine state machine", () => {
    ...
    it("drives a fresh chain deposit from Detected to Done (lock-first)", async () => {
        const storage = createDepositStorage(
            new MemoryKeyValueStore(EPHEMERAL),
        );
        ...
        // The commitment is always locked on the bridge asset, regardless of
        // the deposit's source asset.
        expect(lockup.sponsoredCommitmentLock).toHaveBeenCalledWith(
            expect.objectContaining({ asset: DEPOSIT_BRIDGE_ASSET }),
        );
    });
    ...
    it("refunds the unbound commitment when the quote is rejected", async () => {
        ...
        expect(refund.sponsoredCommitmentRefund).toHaveBeenCalledWith(
            expect.objectContaining({ asset: DEPOSIT_BRIDGE_ASSET }),
        );
