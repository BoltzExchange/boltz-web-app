...
describe("resolveInvoice", () => {
    ...
    test("times out a stalled race", async () => {
        isLnurlMock.mockReturnValue(true);
        fetchLnurlMock.mockReturnValue(new Promise<string>(() => {}));
        fetchBip353Mock.mockReturnValue(new Promise<string>(() => {}));

        await expect(
            resolveInvoice("user@example.com", 1000, { timeoutMs: 20 }),
    ...
    test("forwards a custom dnsOverHttps endpoint as the 3rd arg to fetchBip353", async () => {
        isLnurlMock.mockReturnValue(true);
        fetchLnurlMock.mockRejectedValue(new Error("no lnurlp host"));
        fetchBip353Mock.mockResolvedValue("lni1frombip353");
    ...
    test("aborting the caller signal cancels both race branches", async () => {
        ...
        fetchBip353Mock.mockImplementation(
            (_bip353, _amount, _doh, signal: AbortSignal) =>
    ...
    test("honors a custom timeoutMs for a stalled single LNURL fetch", async () => {
        isLnurlMock.mockReturnValue(true);
        fetchLnurlMock.mockReturnValue(new Promise<string>(() => {}));

        await expect(
            resolveInvoice("lnurl1abc", 1000, { timeoutMs: 5 }),
        ).rejects.toThrow("Timeout");
        expect(fetchLnurlMock).toHaveBeenCalledWith("lnurl1abc", 1000, {
            signal: undefined,
            timeoutMs: 5,
    ...
    test("forwards the amount and default DoH endpoint to fetchBip353", async () => {
        isLnurlMock.mockReturnValue(true);
        fetchLnurlMock.mockRejectedValue(new Error("no lnurlp host"));
        fetchBip353Mock.mockResolvedValue("lni1frombip353");
