...
export class WASMProofBuilder {
    ...
    free() {
        ...
        wasm.__wbg_wasmproofbuilder_free(ptr, 0);
    }
}
if (Symbol.dispose) WASMProofBuilder.prototype[Symbol.dispose] = WASMProofBuilder.prototype.free;
...
export function get_next_query(proof_builder) {
    ...
    const ret = wasm.get_next_query(proof_builder.__wbg_ptr);
    let v1;
    if (ret[0] !== 0) {
...
export function get_unverified_proof(proof_builder) {
    ...
    const ret = wasm.get_unverified_proof(ptr0);
    if (ret[3]) {
...
export function verify_byte_stream(stream, name_to_resolve) {
    ...
    try {
        ...
        const ret = wasm.verify_byte_stream(ptr0, len0, ptr1, len1);
        deferred3_0 = ret[0];
        deferred3_1 = ret[1];
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./dnssec_prover_wasm_bg.js": import0,
    };
}

const WASMProofBuilderFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_wasmproofbuilder_free(ptr, 1));
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        throw new Error('dnssec wasm bytes must be provided to init()');
    }
    const imports = __wbg_get_imports();
