...
// `address`. Keyed on the source asset plus immutable `Transfer` log
// (sourceAsset+txHash+logIndex), NOT on a balance read — a reusable address
// mixes deposits and leftovers, so balance is never a safe identity (P0-1).
// `getLogs` is filtered by the configured token contract, so a wrong-token
// transfer is never returned.
