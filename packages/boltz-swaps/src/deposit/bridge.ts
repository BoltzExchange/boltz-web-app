...
// The guid keys the Forwarded-mint lookup to the source burn tx. `getSentEvent`
// derives it from the burn receipt as `encodeCctpGuid(sourceDomain, txHash)`;
// after a crash-recovery we only have the tx hash, so reconstruct the same guid
// from the source chain's configured CCTP domain (identical to the domain in
// the burn message header).
...
// Gas-sponsored CCTP burn of one deposit on the source chain, minting to
// `mintRecipient` (the same derived address) on Arbitrum in Forwarded mode.
// Persists the recovery record BEFORE broadcasting so a crash mid-send is
// recoverable via `recoverBurn`.
...
// Poll Circle's Forwarding Service for the mint on Arbitrum. The guid is keyed
// to the source burn tx hash, so each deposit resolves its own forward tx.
// Cross-checks the mint recipient (defensive against Circle batching multiple
// mints into one forward tx). Returns undefined on timeout → caller falls back
// to `manualMint`.
...
// `receiveMessage` on the Arbitrum MessageTransmitter ourselves, guarded by
// `isCctpNonceUsed` for idempotency. Gives a 1:1 nonce->mint mapping and
// removes the unbounded-attestation liveness risk. Returns undefined until the
...
// Recover an interrupted burn by rescanning DepositForBurn logs from the
// persisted record.
