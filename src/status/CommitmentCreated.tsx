...
import { validateInvoiceInput, validateResponse } from "../utils/validation";

const lockupAmountToInternal = (asset: string, amount: bigint) =>
...
type CommitmentInvoiceValidation = {
    invoice: string;
...
export const validateCommitmentInvoice = (
    ...
    const { invoice, sats } = validateInvoiceInput(value);
...
const fetchCommitmentInvoice = async (
    validation: CommitmentInvoiceValidation,
): Promise<string> => {
    if (isLnurl(validation.invoice)) {
        return firstResolved(
            [
                fetchLnurl(validation.invoice, validation.sats),
                fetchBip353(validation.invoice, validation.sats),
            ].map((promise) =>
    ...
    const invoice = (
        await fetchBolt12Invoice(validation.invoice, validation.sats)
    ).invoice;
    validateInvoiceForOffer(validation.invoice, invoice);
    return invoice;
};

const resolveCommitmentInvoice = async (
    validation: CommitmentInvoiceValidation,
    amounts: CommitmentAmounts,
): Promise<CommitmentInvoiceValidation> => {
    ...
    return {
        ...validateCommitmentInvoice(
            await fetchCommitmentInvoice(validation),
            amounts,
        ),
...
export const calculateCommittedSubmarineAmounts = async (
    ...
    return {
        boltzFee: directPair.feeOnSend(sendAmount),
        lockupAmount,
        networkFee: BigNumber(directPair.minerFees),
        sendAmount,
        receiveAmount,
    };
};
...
const CommitmentCreated = () => {
    ...
    const [committedAmounts] = createResource(
        ...
        async ({
            ...
            const lockupAmount = lockupAmountToInternal(
                current.assetSend,
                event.amount,
            );
            ...
            return await calculateCommittedSubmarineAmounts(
                directPair,
                lockupAmount,
            );
        },
    ...
    const completeSwap = async () => {
        ...
        setLoading(true);
        try {
            const resolvedInvoice = await resolveCommitmentInvoice(
                validatedInvoice,
                amounts,
            );
