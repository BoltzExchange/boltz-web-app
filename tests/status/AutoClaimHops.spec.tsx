...
import type * as BoltzEvmModule from "boltz-swaps/evm";
...
vi.mock("boltz-swaps/evm", async (importOriginal) => ({
    ...(await importOriginal<typeof BoltzEvmModule>()),
    satsToAssetAmount: (amount: number) => BigInt(amount),
}));

vi.mock("../../src/consts/Assets", async (importOriginal) => ({
    ...(await importOriginal<typeof AssetsModule>()),
    getTokenAddress: () => "0x1000000000000000000000000000000000000000",
}));
...
vi.mock("boltz-swaps/client", async (importOriginal) => ({
    ...
    encodeDexQuote: () => Promise.resolve({ calls: [] }),
}));
        ...
        value: {
            request: vi.fn(
                async (
                    ...
                    optionsOrCallback:
                        | LockOptions
                        | ((lock: Lock | null) => Promise<unknown>),
...
describe("AutoClaimHops", () => {
    ...
    beforeEach(() => {
        ...
        getErc20Swap.mockReturnValue({
            address: "0x5000000000000000000000000000000000000000",
            read: { version: vi.fn().mockResolvedValue(1n) },
        });
