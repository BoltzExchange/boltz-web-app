...
const useExistingRescueKey = async (page: Page, rescueFilePath: string) => {
    await page
...
test.describe("RSK Rescue", () => {
    ...
    const createAndClaimLbtcToRbtcSwap = async (
        page: Page,
        walletClient: WalletClient,
        isFirstSwap: boolean,
    ) => {
        ...
        await selectAssets(page, "L-BTC", "RBTC");
        await connectWallet(page, walletClient);
        ...
        await elementsSendToAddress(lockupAddress, sendAmount);
        await generateLiquidBlock();
        await page.waitForTimeout(1000);
        await generateAnvilBlock();
    ...
    test("lastUsedEvmKey index increments correctly after importing existing rescue file", async ({
        page,
        walletClient,
    }) => {
        ...
        // This sets lastUsedEvmKey for RBTC to 2
        await createAndClaimLbtcToRbtcSwap(page, walletClient, true);
        await createAndClaimLbtcToRbtcSwap(page, walletClient, false);

        const rescueFileContent = fs.readFileSync(rescueFileName, "utf8");

        await clearBrowserStorage(page);

        await page.goto("/");
        await selectAssets(page, "BTC", "L-BTC");
        const lbtcAddress = await getLiquidAddress();
        await page
            .locator("input[data-testid='onchainAddress']")
            .fill(lbtcAddress);
        ...
        // Use existing rescue key
        fs.writeFileSync(rescueFileName, rescueFileContent);
        await useExistingRescueKey(page, rescueFileName);
        await waitForSwapCreated(page);
    });

    test("deferred EVM key index recovery on swap creation when wallet was not connected during import", async ({
        page,
        walletClient,
    }) => {
        test.setTimeout(120_000);
        ...
        // Create a non-RBTC swap without connecting wallet,
        await page.goto("/");
        await selectAssets(page, "BTC", "L-BTC");
        const lbtcAddress = await getLiquidAddress();
        await page
        ...
        const swapCreated = page.locator("div[data-status='swap.created']");

        await expect(swapCreated).toBeVisible();
    });
});
